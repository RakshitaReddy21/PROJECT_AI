      ...
      <div className="hidden lg:flex lg:w-3/5 p-12 flex-col justify-between relative z-10 border-r border-[#F1E8E3]">
        ...
        <div className="my-auto space-y-8 max-w-xl">
          ...
          <div className="relative pt-4">
            <div className="absolute left-4 top-6 bottom-6 w-0.5 bg-gradient-to-b from-[#F29B73] via-[#E9825B] to-[#F8C9B0] opacity-40" />
              ...
              {journeySteps.map((stepItem, idx) => {
                ...
                return (
                  <div key={stepItem.label} className="flex items-center space-x-4 relative z-10 group">
                    <div className="w-8 h-8 rounded-xl bg-white border border-[#F1E8E3] text-[#E9825B] flex items-center justify-center shadow-sm group-hover:border-[#F29B73] transition-all">
                      <StepIcon className="w-4 h-4" />
                    </div>
                    <div className="flex-1 p-3 bg-white/80 border border-[#F1E8E3] rounded-xl group-hover:border-[#F8C9B0] transition-all shadow-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-mono font-bold text-[#292524] tracking-wider">
                          0{idx + 1}. {stepItem.label}
                        </span>
                        <span className="text-[10px] font-mono text-[#E9825B] font-semibold">Active Engine</span>
                      </div>
                      <p className="text-[11px] text-[#78716C] mt-0.5">{stepItem.desc}</p>
                    </div>
                  </div>
                );
              })}
      ...
      <div className="w-full lg:w-2/5 p-6 md:p-12 flex items-center justify-center relative z-10">
        <div className="w-full max-w-md space-y-6">
          ...
          <Card className="p-7 space-y-5 bg-white border border-[#F1E8E3] rounded-2xl shadow-xl">
            ...
            <div className="flex bg-[#FFF8F5] p-1 rounded-xl border border-[#F1E8E3] text-xs font-mono">
              ...
              <button
                ...
                className={`flex-1 py-2 rounded-lg font-semibold transition-all flex items-center justify-center gap-1.5 ${
                  loginMode === 'admin'
                    ? 'bg-[#FFF0E8] text-[#E9825B] border border-[#F8C9B0] shadow-sm'
                    : 'text-[#78716C] hover:text-[#292524]'
                }`}
              >
                <ShieldCheck className="w-3.5 h-3.5 text-[#E9825B]" /> Admin Portal
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              {loginMode === 'admin' && (
                <div className="p-2.5 bg-[#FFF0E8] border border-[#F8C9B0] rounded-xl text-[11px] font-mono text-[#E9825B] flex items-center gap-2">
              ...
              <Button
                variant="signal"
                type="submit"
                className="w-full py-3 font-semibold text-xs uppercase tracking-wider bg-[#F29B73] hover:bg-[#E9825B] text-white shadow-sm"
          ...
          <p className="text-center text-xs text-[#78716C]">
            ...
            <Link to="/register" className="text-[#E9825B] font-semibold hover:underline">
