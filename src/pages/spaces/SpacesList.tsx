  ...
  return (
    <PageTransition className="space-y-6 max-w-6xl mx-auto pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#F1E8E3]">
        ...
        <Button variant="signal" onClick={() => setIsModalOpen(true)} className="gap-2 font-semibold bg-[#F29B73] hover:bg-[#E9825B] text-white shadow-sm">
          <FolderPlus className="w-4 h-4" /> Create Space
        </Button>
      </div>

      {spaces.length === 0 ? (
        <div className="p-12 text-center bg-white rounded-2xl border border-[#F1E8E3] space-y-4 max-w-lg mx-auto my-8 relative overflow-hidden shadow-sm">
          <div className="p-4 bg-[#FFF0E8] text-[#E9825B] border border-[#F8C9B0] rounded-2xl w-14 h-14 mx-auto flex items-center justify-center shadow-sm">
            <Compass className="w-8 h-8" />
          </div>
          <div className="space-y-1">
            <h3 className="font-display text-xl font-semibold text-[#292524]">Your Learning Universe is Empty</h3>
